...
// можно в качестве типа elems использовать map, тогда проверка вхождения будет осуществлятся проще
// elems := map[string]int
// elems[elem] += 1
// _, ok := elems[elem] - проверка вхождения в ok будет true, если элемент с таким ключом есть
